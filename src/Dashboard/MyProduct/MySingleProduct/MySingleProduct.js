...
const MySingleProduct = ({ mySinglePrd, refetch }) => {
  ...
  const myProduct = () => {
    ...
    if (agree) {
      fetch(`https://dream-bike-server-rose.vercel.app/myProduct/${_id}`, {
        method: "DELETE",
        headers: {
  ...
  return (
    <tr>
      ...
      <th className="text-center">{soldOut ? <button>Sold</button> : <button>Advertise</button>}</th>
      <th className="text-center">
        {soldOut ? (
          <button>Delivered</button>
        ) : (
          <>
            <button onClick={myProduct} className="bg-red-600 p-3 rounded text-white">
              Delete
            </button>
